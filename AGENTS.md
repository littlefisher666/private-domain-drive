...
一期为控制 FC 调用频率与运行成本，采用客户端本地缓存 `stsBroker` 并直连阿里云 STS 刷新临时凭证的方案。客户端发布构建同时注入 FC HTTP 请求签名所需配置；服务端仅在首次登录和必要控制面调用时参与凭证签发，不承担每次 STS 刷新的中转。

这是当前已确认的架构取舍。除非用户明确要求安全评审、凭证架构调整或相关实现变更，不要将客户端持有上述配置作为重复的风险提示或建议服务端托管长期凭证。
