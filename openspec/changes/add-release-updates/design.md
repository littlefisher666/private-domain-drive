...
客户端以 GitHub Releases 进行非应用商店分发。当前发布工作流已生成 APK 和 macOS ZIP，但客户端没有更新检查能力，设置页与 FC 请求使用硬编码版本。Android 可以在用户确认后安装同签名的新 APK；macOS 因无法提供 Developer ID 签名和公证，不能可靠地在应用内替换自身。
...
- 以公开 GitHub Releases API 为稳定版更新元数据源，并选择当前平台的发布资产。
...
### 直接读取 GitHub Latest Release

更新仓库通过公开 `GET /repos/{owner}/{repo}/releases/latest` 读取。客户端仅接受非 draft、非 prerelease 的结果，并通过确定的资产名匹配当前平台：Android 为 `private-domain-drive-android-v{version}.apk`，macOS 为 `private-domain-drive-macos-v{version}.dmg`。tag 中可选的 `v` 前缀在 SemVer 比较前移除。

GitHub 返回的 asset `digest` 是 APK 的 SHA-256 来源；下载完成后必须先校验，才允许安装。备选的静态 JSON 清单会增加额外发布同步步骤，不采用。
...
- [公开 GitHub API 受限流或网络影响] → 检查失败不影响正常使用，向用户展示可重试的简短错误。
- [Release 被误发布或资产名不一致] → 严格筛选平台资产，无可用资产时不提示可更新。
- [APK 下载被篡改或中断] → 必须匹配 GitHub API 提供的 SHA-256，失败后删除临时文件且不发起安装。
...
4. 若发布异常，删除或标记对应 GitHub Release，旧版本不受更新检查失败影响。
